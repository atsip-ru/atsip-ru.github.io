...
[Источник](https://meshtastic.org/blog/zero-cost-hops-favorite-routers/)

Что если бы сообщения могли проходить через вашу сетевую инфраструктуру в один хоп?
Что, если бы вы могли связать два города в 900 километрах друг от друга через LoRa и *все еще* оставались хопы в запасе?
...
![Проклятое и благословенное изображение игры в классики с множеством 3 и 4, вписанных в квадраты](https://meshtastic.org/assets/images/zero_cost_hopscotch-d37d0ea2146c46f30f72e4af6b73ec91.png)
...
- Быстрые предустановки, такие как `SHORT_FAST` и `SHORT_TURBO`, жертвуют дальностью ради скорости. В плотных городских сетях это означает,
что 7-хоповый предел можно исчерпать очень быстро.
- **Zero-Cost Hops** превращают сеть из тщательно размещённых инфраструктурных маршрутизаторов в один хоп, оставляя больше хопов
для первой и последней мили.
...
- Помогает предотвратить превращение `CLIENT_BASE` в последний хоп пакета, чтобы сообщения не «умирали» на крыше до того, как достигнут
ваши внутренние ноды.
...
- Правила ретрансляции `CLIENT_BASE` остаются точно такими же. `CLIENT_BASE` использует `from` / `to`, в то время как Zero-Cost Hops
использует `relay_node`.
...
Чтобы protobuf-файлы оставались маленькими и экономили память, нода сопоставляет **relay_node** с вашим списком избранного и проверяет,
что нода является инфраструктурной.
...
| Общее количество нод | Маршрутизаторы (3% от количества нод) | Избранные маршрутизаторы в зоне досягаемости (20% от маршрутизаторов) | Вероятность коллизии |
| -------------------- | ------------------------------------- | --------------------------------------------------------------------- | -------------------- |
| 10                   | 0.3 (≈1)                              | 1                                                                     | ~0.4 %               |
| 50                   | 1.5 (≈2)                              | 1                                                                     | ~0.4 %               |
| 100                  | 3                                     | 1                                                                     | ~0.4 %               |
...
*Примечание: «Общее количество нод» — это размер сети; вероятность коллизии рассчитывается на основе числа избранных маршрутизаторов
в зоне действия, а не всех нод, делённых на 256. Например, при 1000 нодах и 6 избранных маршрутизаторах вероятность того, что случайная
нода-посредник совпадёт с избранной в зоне действия, составляет около 2,3%.*

Вероятность коллизии здесь низка.
...
- Ноды на возвышенностях используют роль `CLIENT_BASE` и выбирают в избранные локальные ноды `ROUTER`.
...
- `CLIENT_BASE` дома; добавляйте в избранное свои портативные устройства.
...
- Наиболее эффективно для нод на больших высотах, поэтому злоупотребления маловероятны при хорошо управляемых сетях.
...
Для входящего трафика, полученного от маршрутизаторов, да. Это предотвращает потерю сообщений на крыше до того, как они доберутся
до ваших внутренних нод.
...
Для Bay Mesh имеется более 700 нод между Тахо и Сан-Луис-Обиспо. При аккуратном размещении маршрутизаторов возможно передавать сообщения
на расстояние более 300 миль, что на LoRa достигается только с помощью zero-cost hops, учитывая рельеф и расстояние.

Используя [Meshview](https://github.com/pablorevilla-meshtastic/meshview), размещённый на [bayme.sh](https://meshview.bayme.sh/),
вы можете наблюдать работу zero-cost hops между Сан-Франциско, Сан-Луис-Обиспо и Тахо.
...
Выберите две или три стратегических инфраструктурные ноды и отметьте их как избранные друг для друга. Наблюдайте, как хопы не расходуются
между маршрутизаторами. Сделайте то же самое для вашей ноды на крыше `CLIENT_BASE` и получите больше сообщений, чем раньше.
