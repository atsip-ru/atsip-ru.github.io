...
| ShortFast     | 250                 | 7      | 10.94                    | 143dB         | Очень быстрой передачи на короткие расстояния  |
| ShortTurbo    | 500                 | 7      | 21.88                    | 140dB         | Максимальная скорость с минимальной дальностью |
...
## Преимущества предустановок с высокой пропускной способностью

Переход на предустановки с более высокой пропускной способностью, такие как MediumSlow, MediumFast, ShortSlow или даже ShortFast, предлагает ряд
преимуществ:
...
уменьшается.
3. **Лучшая масштабируемость**: предустановки с высокой пропускной способностью предназначены для работы с большим количеством нод и более высоким
объемом сообщений, что делает их более подходящими для крупных сетей. В плотных сетях улучшенная пропускная способность часто с лихвой компенсирует
слегка уменьшенную дальность, что приводит к более надежной доставке сообщений.
...
> График водопада SDR пакетов на нескольких различных пресетах (слева направо): LongFast, MediumFast, ShortFast
![Presets](/_resources/longfast-1.webp)
...
**ShortFast** или **ShortSlow** - эти настройки обеспечивают максимально высокие скорости передачи данных, значительно снижая перегрузку
...
**MediumFast** или **MediumSlow** — эти установки обеспечивают отличное сочетание дальности и скорости, предлагая скорость передачи данных
...
3. **Приложение для Android/iOS**: Перейдите в Настройки > Настройки устройства > LoRa > Шаблоны

Помните: все ноды в вашей сети должны использовать одну и ту же установку, чтобы оставаться в сети.
...
Meshtastic Bay Area — это mesh-сообщество в районе залива Сан-Франциско, которое переключило свою настройку на MediumSlow. Вот их отзыв об этом
изменении:

Наша сеть из более чем 150 нод в настоящее время процветает после перехода на настройку MediumSlow, которая оказалась чрезвычайно полезной. Всё
началось как эксперимент, чтобы выйти из перегруженного канала по умолчанию LongFast, в котором было много нод с устаревшей прошивкой или
неправильно настроенных нод, засоряющих сеть.
...
2. **Меньше фонового шума:** MediumSlow не является стандартной предустановкой, поэтому мы избегаем старого трафикового хаоса, накопленного за годы
...
дублирования. Высота имеет значение и мы используем её в наших интересах. Иногда мы размещаем 2 маршрутизатора на противоположных концах
...
и проверять предположения. Это гарантирует, что производительность и поведение mesh-сети понятны, а изменения подкреплены результатами и
повторяемой наукой.
...
части Северного острова Новой Зеландии и часть северного участка Южного острова. Население распределено по сложной местности, что может затруднять
...
nodeinfo, местоположений и обновлений телеметрии от клиентов. Большинство функций, включая текстовые сообщения, были крайне ненадежными - и
...
конечном итоге решили, что SHORT_FAST будет лучшим вариантом для долгосрочной стратегии, при условии, что это будет реально осуществимо. Перевод
...
медленно для будущего роста. Первоначальный тест прошел успешно.

Как только концепция была подтверждена, мы организовали тестовый день, в ходе которого перевели все ключевые высотные ноды региона на SHORT_FAST
...
что пользователям нужно было изменить только одну настройку со значения по умолчанию, чтобы подключиться к нашей сети). О переходе было
...
невозможно покрыть. С декабря 2024 года мы развернули несколько нод ROUTER_LATE для покрытия этих зон — эти ноды гарантируют, что наши бывшие
«черные зоны» теперь имеют доступ в более широкую сеть, и оказались чрезвычайно полезными.

### Как уговорить других переключиться / Как справиться с FOMO

Если вы состоите в сообществе или группе, где многие все еще используют LongFast, подумайте о том, чтобы поделиться своим опытом и преимуществами,
которые вы заметили. Вы также можете помочь им понять преимущества, связанные с переходом на другие настройки. Это может быть отличной возможностью
...
крупные или плотные развертывания часто получают значительную выгоду от перехода на установки с более высокой пропускной способностью.
...
особенно в сценариях, когда ноды располагаются относительно близко друг к другу.
